...
def matmul(a, b, c, chip, precision):
    """Calculate number of FLOP and duration of a matmul of shape (a x b) x (b x c)."""
    flop = 2*a*b*c
    mem_io = a*b + b*c + a*c
    
    flop_time = flop / chip['flop_per_sec_8bit'] * (precision/8)
...
chip_types = dict(H100=dict(flop_per_sec_8bit=1979e12, io=3.35e12, net=900e9),
                  ...
                  H100_x64_pod=dict(flop_per_sec_8bit=64*1979e12, io=64*3.35e12, net=125e8))
...
b, m, l, prec, L, d, d_internal_ratio, e, k, D, chip_type, pp, pipeline_interleaving, dp, tp, tp2 = \
...
cluster_flop_per_sec = p * chip['flop_per_sec_8bit'] * (8/prec)
...
assert L % (pp * pipeline_interleaving) == 0, 'need integer number of virtual pipeline stages'
...
print(f'Parameters: {P:.2e}, Training Tokens: {D:.2e}')
print(f'{p} {chip_type}s')
...
_, matmul_time = matmul(d_int_per_matmul, d_per_matmul, tokens_per_matmul, chip, prec)
...
# Determine pipeline full time.
#
# Note: We assume communication at this point is fully overlapped with
# computation.
pipeline_full_time = 3 * (L//pp) * matmuls_per_chip_per_layer * matmul_time * n_microbatches_per_batch
...
print(f'Pipeline full time:   {pipeline_full_time:9.4f} secs')
print(f'Gradient reduction:   {gradient_reduction_time:9.4f} secs')
print(f'Total batch time:     {batch_time:9.4f} secs')

util = flop_per_batch / (cluster_flop_per_sec * batch_time)
print()
print(f'Utilization:          {util:9.4f}')

print()
print('TOTAL RUN TIME')
print(f'{batch_time * n_batches / 86400:6.1f} days')
